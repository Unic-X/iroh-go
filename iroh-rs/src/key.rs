...
impl SecretKey {
    ...
    pub fn from_bytes(bytes: Vec<u8>) -> Result<Self, IrohError> {
        ...
        Ok(
           SecretKey{
            inner : iroh::SecretKey::from_bytes(&bytes)})
    }
    ...
    pub fn public(&self) -> Arc<EndpointId> {
        Arc::new(self.inner.public().into())
    }
...
impl Signature {
    ...
    pub fn from_bytes(bytes: Vec<u8>) -> Result<Self, IrohError> {
        ...
        Ok(Signature{
            inner: iroh_base::Signature::from_bytes(&bytes)})
    }
